...
namespace shoecomp
{
    ...
    void ImageCanvas::renderCanvas(
        ...
        AnnotationMode* pMode)
    {
        AnnotationMode modeVal =
            pMode ? *pMode : AnnotationMode::None;
            ...
            io.MouseWheel != 0.0f)
        {
            ...
            if (linked)
                linked->panTarget.y = vs.panTarget.y;
        }
            ...
            !io.KeyCtrl && mode != AnnotationMode::None)
        {
            ImVec2 ic = screenToImageCoord(
                io.MousePos, canvasPos, avail, vs.pan,
                vs.zoom, baseScale, vs.rotation,
        ...
        vs.rotation +=
            (vs.rotationTarget - vs.rotation) * speed;
        ...
        ImVec2 br = rot(hw, hh);
        ImVec2 bl = rot(-hw, hh);
        dl->AddImageQuad(img.textureId, tl, tr, br, bl,
                ...
                img.annotations["bounds"].isArray())
            {
                ...
                if (bnd.size() >= 2)
                {
                    ImVec2 last = imageToScreenCoord(
                        bnd.back()["x"].getFloat(),
                        bnd.back()["y"].getFloat(), cx,
                        cy, annScale, cosR, sinR,
                        img.width, img.height);
                    ImVec2 first = imageToScreenCoord(
                        bnd[0]["x"].getFloat(),
                        bnd[0]["y"].getFloat(), cx, cy,
                        annScale, cosR, sinR,
                        img.width, img.height);
                    if (!editing)
                    {
    ...
    void ImageCanvas::renderToolbar(
        LoadedImage& img, ImageViewState& vs,
        const char* toolbarId, AnnotationMode* mode,
        ImageViewState* linked)
    {
        ...
        ImGui::SameLine();
        ImGui::Text("%.0f deg",
                    vs.rotation * 180.0f / 3.14159265f);
