...
namespace shoecomp
{
        ...
        jt::Json& annotations)
    {
        ...
        if (!parsed.isObject())
            return -1;
        if (!parsed.contains("bounds")
            || !parsed["bounds"].isArray())
            return -1;
        if (!parsed.contains("points")
            || !parsed["points"].isArray())
            return -1;

        for (size_t i = 0;
             i < parsed["bounds"].getArray().size();
             ++i)
        {
